...
const DANGEROUS_PATTERNS = [
  ...
  { pattern: /fetch\s*\(\s*(['"`])https?:\/\/(?!cdn\.jsdelivr\.net|cdnjs\.cloudflare\.com|fonts\.(googleapis|gstatic)\.com)/gi, reason: 'external fetch' },
  ...
  { pattern: /\.src\s*=\s*(['"`])https?:\/\/(?!cdn\.jsdelivr\.net|cdnjs\.cloudflare\.com)/gi, reason: 'external script source' },
...
export function sanitizeAIOutput(code) {
  ...
  // Remove new Function
  cleaned = cleaned.replace(/new\s+Function\s*\(/gi, '/* [blocked: Function constructor] */ (function(){return function(){};})(');
