...
export const ENGINE_FAMILY_PROFILES = {
  ...
  puzzleCasual: {
    ...
    heroTemplateIds: ['matching-game', 'memory-card-game', 'chess'],
    starterTemplateIds: ['matching-game', 'memory-card-game', 'chess'],
  ...
  towerDefense3d: {
    engineId: ENGINE_IDS.VIBE_3D,
    dimension: '3d',
    label: 'Tower Defense 3D',
    defaultTemplateGenre: null,
    heroTemplateIds: ['tower-defense-3d'],
    starterTemplateIds: ['tower-defense-3d'],
    modelPackHint: 'tower-defense-3d',
    validationProfile: 'vibe-3d-tower-defense',
    coreSystems: [
      'battlefield terrain',
      'winding enemy path',
    ...
    safeEditSurfaces: ['tower types', 'enemy types', 'map layout', 'wave pacing', 'upgrade costs'],
    architecture: createArchitecture('3d', {
      inputModel: 'click or tap glowing build pads or placement zones, wave start button',
      cameraModel: 'top-down or isometric camera overlooking the battlefield',
      hudContract: 'wave counter, gold, lives, and tower selection visible at all times',
      failureContract: 'base destroyed overlay with wave reached and replay option',
    }),
  },
  minigolf3d: {
    engineId: ENGINE_IDS.VIBE_3D,
    ...
    label: 'Minigolf 3D',
    defaultTemplateGenre: null,
    heroTemplateIds: ['minigolf-3d'],
    starterTemplateIds: ['minigolf-3d'],
    modelPackHint: 'minigolf-3d',
    validationProfile: 'vibe-3d-minigolf',
    coreSystems: ['aim system', 'power meter', 'ball physics', 'hole detection', 'stroke counter', 'course layout'],
    safeEditSurfaces: ['course layout', 'obstacle types', 'ball color', 'theme', 'par values'],
    architecture: createArchitecture('3d', {
      inputModel: 'click-drag to aim and set power, release to putt',
      cameraModel: 'orbit camera following the ball with hole overview on reset',
      hudContract: 'stroke count, par, hole number, and power meter always visible',
      failureContract: 'ball reset after out-of-bounds with stroke penalty feedback',
    }),
  },
  marbleRun3d: {
    engineId: ENGINE_IDS.VIBE_3D,
    ...
    label: 'Marble Run 3D',
    defaultTemplateGenre: null,
    heroTemplateIds: ['marble-run-3d'],
    starterTemplateIds: ['marble-run-3d'],
    modelPackHint: 'marble-run-3d',
    validationProfile: 'vibe-3d-marble-run',
    coreSystems: ['marble physics', 'track segments', 'gravity flow', 'finish detection', 'build mode'],
    safeEditSurfaces: ['track layout', 'segment types', 'marble color', 'theme', 'speed tuning'],
    architecture: createArchitecture('3d', {
      inputModel: 'click to place track segments in build mode, release marble button to start',
      cameraModel: 'orbit camera with follow mode during marble run',
      hudContract: 'segment inventory, timer, and completion feedback',
      failureContract: 'marble falls off track with retry and edit prompt',
    }),
  },
  kartRacing3d: {
    engineId: ENGINE_IDS.VIBE_3D,
    dimension: '3d',
    label: 'Kart Racing 3D',
    defaultTemplateGenre: null,
    heroTemplateIds: ['kart-racer-3d'],
    starterTemplateIds: ['kart-racer-3d'],
    modelPackHint: 'kart-racing-3d',
    validationProfile: 'vibe-3d-kart-racing',
    coreSystems: ['kart driving rig', 'lap system', 'item pickups', 'boost mechanic', 'AI opponents'],
    safeEditSurfaces: ['kart body', 'track layout', 'item types', 'boost tuning', 'opponent count'],
    architecture: createArchitecture('3d', {
      inputModel: 'accelerate, brake, steer, and use-item controls with touch fallback',
      cameraModel: 'chase camera behind the kart with drift-aware offset',
      hudContract: 'lap counter, position, speed, item slot, and boost meter',
      failureContract: 'finish or DNF overlay with time and replay option',
    }),
  },
  coasterPark3d: {
    engineId: ENGINE_IDS.VIBE_3D,
    dimension: '3d',
    label: 'Coaster Park 3D',
    defaultTemplateGenre: null,
    heroTemplateIds: ['coaster-park-3d'],
    starterTemplateIds: ['coaster-park-3d'],
    modelPackHint: 'coaster-park-3d',
    validationProfile: 'vibe-3d-coaster-park',
    coreSystems: ['track builder', 'coaster physics', 'ride camera', 'park economy', 'visitor system'],
    safeEditSurfaces: ['track layout', 'coaster type', 'park decorations', 'stall placement', 'ride speed'],
    architecture: createArchitecture('3d', {
      inputModel: 'click to place track segments and park objects, ride button to test',
      cameraModel: 'top-down park view in build mode, first-person ride-along in test mode',
      hudContract: 'money, visitor count, excitement rating, and track piece palette',
      failureContract: 'track incomplete warning with edit prompt, budget depletion feedback',
    }),
  },
  medievalVillage3d: {
    engineId: ENGINE_IDS.VIBE_3D,
    dimension: '3d',
    label: 'Medieval Village 3D',
    defaultTemplateGenre: null,
    heroTemplateIds: ['medieval-village-3d'],
    starterTemplateIds: ['medieval-village-3d'],
    modelPackHint: 'medieval-village-3d',
    validationProfile: 'vibe-3d-medieval-village',
    coreSystems: ['placement grid', 'building system', 'villager spawns', 'resource loop', 'quest system'],
    safeEditSurfaces: ['building types', 'village layout', 'villager roles', 'resource values', 'quest text'],
    architecture: createArchitecture('3d', {
      inputModel: 'click to place buildings and roads, interact with villagers',
      cameraModel: 'isometric or orbit camera with zoom for village overview',
      hudContract: 'resources, population, building palette, and quest log',
      failureContract: 'soft pressure from resource depletion with recovery path',
    }),
  },
};
...
export const TEMPLATE_BLUEPRINTS = {
  ...
  'tower-defense-3d': {
    id: 'tower-defense-3d',
    label: 'Tower Defense 3D',
    family: 'towerDefense3d',
    sourceTemplateGenre: null,
    templateFile: 'tower-defense-3d.html',
    dimension: '3d',
  },
  'minigolf-3d': {
    id: 'minigolf-3d',
  ...
  'marble-run-3d': {
    ...
    family: 'marbleRun3d',
    sourceTemplateGenre: null,
    templateFile: 'marble-run-3d.html',
    dimension: '3d',
  },
  'kart-racer-3d': {
    id: 'kart-racer-3d',
    ...
    family: 'kartRacing3d',
    sourceTemplateGenre: null,
    templateFile: 'kart-racer-3d.html',
    dimension: '3d',
  },
  'coaster-park-3d': {
    id: 'coaster-park-3d',
    label: 'Coaster Park 3D',
    family: 'coasterPark3d',
    sourceTemplateGenre: null,
    templateFile: 'coaster-park-3d.html',
    dimension: '3d',
  },
  'medieval-village-3d': {
    id: 'medieval-village-3d',
    label: 'Medieval Village 3D',
    family: 'medievalVillage3d',
    sourceTemplateGenre: null,
    templateFile: 'medieval-village-3d.html',
    dimension: '3d',
  },
};
...
const GAME_TYPE_ALIASES = {
  ...
  'tower-defense-game-3d': 'tower-defense-3d',
  '3d-tower-defense': 'tower-defense-3d',
  minigolf: 'minigolf-3d',
  'mini-golf': 'minigolf-3d',
  'mini-golf-3d': 'minigolf-3d',
  'putt-putt': 'minigolf-3d',
  'marble-run': 'marble-run-3d',
  'marble-race': 'marble-run-3d',
  'marble-maze': 'marble-run-3d',
  'kart-racer': 'kart-racer-3d',
  'kart-racing': 'kart-racer-3d',
  'go-kart': 'kart-racer-3d',
  'mario-kart': 'kart-racer-3d',
  'coaster-park': 'coaster-park-3d',
  'roller-coaster': 'coaster-park-3d',
  'theme-park': 'coaster-park-3d',
  'theme-park-tycoon': 'coaster-park-3d',
  'amusement-park': 'coaster-park-3d',
  'medieval-village': 'medieval-village-3d',
  'fantasy-town': 'medieval-village-3d',
  'village-builder': 'medieval-village-3d',
  'town-builder': 'medieval-village-3d',
  'castle-builder': 'medieval-village-3d',
};
...
const GENRE_FAMILY_BY_GAME_TYPE = {
  ...
  'tower-defense-3d': 'towerDefense3d',
  'minigolf-3d': 'minigolf3d',
  'marble-run-3d': 'marbleRun3d',
  'kart-racer-3d': 'kartRacing3d',
  'coaster-park-3d': 'coasterPark3d',
  'medieval-village-3d': 'medievalVillage3d',
};
...
function inferStarterTemplateFromPrompt(prompt = '') {
  ...
  const starterSignals = [
    ...
    {
      id: 'chess',
      patterns: [/\bchess\b/i, /\bchess board\b/i],
    },
    {
      id: 'trick-shot-arena',
      patterns: [
...
export function resolveEngineProfile({
  ...
  const explicitPromptFamily =
    ...
    detectedFamily ||
    null;
  const explicitPromptShouldOverrideIteration =
    !!explicitPromptFamily &&
    !!normalizedRequestedType &&
    (!codeDetectedFamily || explicitPromptFamily !== codeDetectedFamily);
  ...
  const iterationLockedFamily = isIterating
    ? explicitPromptShouldOverrideIteration
      ? explicitPromptFamily
      : codeDetectedFamily
    : null;
