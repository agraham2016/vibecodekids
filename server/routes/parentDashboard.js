...
function findChildrenByParentEmail(users, parentEmail) {
  return users.filter(u =>
    u.parentEmail && u.parentEmail.toLowerCase() === parentEmail.toLowerCase() &&
    u.status !== 'deleted'
  );
}
...
router.get('/auth', (req, res) => {
  ...
  if (!session || Date.now() > session.expiresAt) {
    ...
    return res.status(401).send('<html><body><h2>Link expired or invalid.</h2><p>Please request a new dashboard link.</p></body></html>');
  }
