...
export default function createBillingRouter(sessions) {
  ...
  router.post('/checkout', async (req, res) => {
    try {
      ...
      const usernameCheck = filterContent(username, { source: 'billing' });
      const displayNameCheck = filterContent(displayName, { source: 'billing' });
  ...
  router.get('/success', async (req, res) => {
    ...
    try {
      ...
      if (needsConsent && parentEmail) {
        ...
        console.log(`Under-13 paid signup: ${username} → consent email sent`);
